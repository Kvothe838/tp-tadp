...
class CaraOCrus(val cruz: Boolean = false) extends TipoApuesta {
...
class ParOImpar(val impar: Boolean = false) extends TipoApuesta {
  override def probabilidad(): Double = 0.5
...
class NegroORojo(val negros: Boolean = false) extends TipoApuesta {
...
class Numero(val n:Int ) extends TipoApuesta {
...
class Docena(val docena: Int) extends TipoApuesta {
